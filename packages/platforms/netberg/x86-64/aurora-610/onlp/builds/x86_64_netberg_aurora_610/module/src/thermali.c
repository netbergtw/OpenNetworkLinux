...
onlp_thermali_info_get(onlp_oid_id_t id, onlp_thermal_info_t* info)
{
    ...
    *info = __onlp_thermal_info[id];
    ...
    if( (id>=ONLP_THERMAL_CPU_PHY)&&(id<=ONLP_THERMAL_CPU_CORE3) ) {
        core_thermal_path=hwmon_path(NET_CTMP_BASE);
        ...
        board_thermal_path=hwmon_path(NET_HWMON_BASE);
        ...
        snprintf(path,ONLP_CONFIG_INFO_STR_MAX,"%sthermal_psu%d", hwmon_path(NET_HWMON_BASE), psu_id);

    } else if( (id==ONLP_THERMAL_2_ON_PSU1)||(id==ONLP_THERMAL_2_ON_PSU2) ) {
        int psu_id=ONLP_OID_ID_GET(info->hdr.poid);
        snprintf(path,ONLP_CONFIG_INFO_STR_MAX,"%sthermal2_psu%d", hwmon_path(NET_HWMON_BASE), psu_id);
    } else {
