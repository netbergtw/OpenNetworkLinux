...
onlp_sfpi_is_present(onlp_oid_id_t port)
{
    ...
    ONLP_TRY( onlp_sfpi_port_type(port) );
    int present_data;
    ONLP_TRY( onlp_file_read_int(&present_data, NET_SFP_PREFIX"port%d/present", port ) );
    switch(present_data) {
    case 1:
    case 0:
        return !present_data;
...
onlp_sfpi_is_rx_los(onlp_oid_id_t port)
{
    ...
    if(p_type == ONLP_PORT_TYPE_SFP) {
        ONLP_TRY( onlp_file_read_int(&rxlos, NET_SFP_PREFIX"port%d/rxlos", port) );
        if(rxlos == 0) {
            rv = false;
        } else {
            rv = true;
        ...
        ONLP_TRY( onlp_file_read((uint8_t*)buf, ONLP_CONFIG_INFO_STR_MAX, &len, NET_SFP_PREFIX"port%d/soft_rx_los", port) );
        if(sscanf( buf, "0x%x\n", &rxlos) != 1) {
