...
extern int net_fani_status_get(onlp_oid_id_t id, uint32_t* rv);
...
static int _sys_onlp_ledi_info_get(onlp_oid_id_t id, onlp_led_info_t* info)
{
    ...
    *info = __onlp_led_info[id];
    ...
    ONLP_TRY(onlp_file_read_int((int*)&grn_mode, NET_SYSLED_PREFIX"grn_led"));
    ONLP_TRY(onlp_file_read_int((int*)&red_mode, NET_SYSLED_PREFIX"red_led"));
...
static int _fan_onlp_ledi_info_get(onlp_oid_id_t id, onlp_led_info_t* info)
{
    ...
    *info = __onlp_led_info[id];
    ...
    if(ONLP_OID_STATUS_FLAG_IS_SET(hdr,PRESENT)) {
        char* info_path=hwmon_path(NET_HWMON_BASE);
...
int net_ledi_status_get(onlp_oid_id_t id, uint32_t* rv)
{
    ...
    case ONLP_LED_FAN3:
    case ONLP_LED_FAN4:
        result=net_fani_status_get(fan_id, &fan_status);
        if(result!=ONLP_STATUS_OK) {
            return result;
        }
...
int onlp_ledi_hdr_get(onlp_oid_id_t id, onlp_oid_hdr_t* hdr)
{
    ...
    *hdr = info->hdr;
    ...
    result = net_ledi_status_get(id, &hdr->status);
