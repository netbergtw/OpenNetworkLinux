...
extern int net_psui_status_get(onlp_oid_id_t id, uint32_t* rv);
...
int net_fani_status_get(onlp_oid_id_t id, uint32_t* rv)
{
    ...
    if( (id>=ONLP_FAN_1) && (id<=ONLP_FAN_4) ) {
        ...
        result=net_psui_status_get( ONLP_OID_ID_GET(hdr->poid), &psu_status) ;
        if(result!=ONLP_STATUS_OK) {
            return result;
        }
...
static int _fani_status_failed_check(uint32_t* status, int id)
{
    ...
    case ONLP_FAN_PSU_2:
        ...
        if( rpm <= 0 || pwm <=0 || pwm > MAX_PWM) {
            uint32_t psu_state;
            rv= net_psui_status_get(psu_id, &psu_state);
            if(rv!=ONLP_STATUS_OK) {
                return rv;
            }
...
int onlp_fani_hdr_get(onlp_oid_id_t id, onlp_oid_hdr_t* hdr)
{
    ...
    info = &__onlp_fan_info[id];
    *hdr = info->hdr;
    result = net_fani_status_get(id, &(hdr->status));
